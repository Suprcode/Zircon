...
namespace LibraryEditor
{
    public class WeMadeLibrary
    {
        ...
        WeMadeLibrary shadowLibrary = null;
        WeMadeLibrary overlayLibrary = null;

        private bool DontFindExtra;

        public WeMadeLibrary(string name, bool dontFindExtra = false)
        {
        ...
        public void Initialize()
        {
            ...
            if (!DontFindExtra)
            {
                ...
                if (File.Exists(shadowPath))
                    shadowLibrary = new WeMadeLibrary(shadowPath, true);

                string overlaypath = fname.Replace(_MainExtention, "_C" + _MainExtention);

                overlayLibrary = null;
                if (File.Exists(overlaypath))
                    overlayLibrary = new WeMadeLibrary(overlaypath, true);
            }
        ...
        public void ToMLibrary()
        {
            ...
            try
            {
                ...
                //for (int i = 0; i < Images.Length; i++)
                {
                    ...
                    WeMadeLibrary shadowimage = shadowLibrary != null ? shadowLibrary : null;
                    WeMadeLibrary overlayimage = overlayLibrary != null ? overlayLibrary : null;

                    bool hasShadow = shadowimage != null && shadowimage.Images != null && i < shadowimage.Images.Length && shadowimage.Images[i] != null && shadowimage.Images[i].Image != null && shadowimage.Images[i].Image.Width > 0 && shadowimage.Images[i].Image.Height > 0;
                    bool hasOverlay = overlayimage != null && overlayimage.Images != null && i < overlayimage.Images.Length && overlayimage.Images[i] != null && overlayimage.Images[i].Image != null && overlayimage.Images[i].Image.Width > 1 && overlayimage.Images[i].Image.Height > 1;

                    if (hasOverlay && !hasShadow)
                    {
                        //image + overlay only
                        library.Images[i] = new Mir3Library.Mir3Image
                        (Images[i].Image,
                        null,
                        hasOverlay ? overlayimage.Images[i].Image : null
                        )
                        {
                            OffSetX = Images[i].X,
                            OffSetY = Images[i].Y,
                            ShadowOffSetX = Images[i].ShadowX,
                            ShadowOffSetY = Images[i].ShadowY,
                            OverlayWidth = hasOverlay ? (short)overlayimage.Images[i].Width : (short)0,
                            OverlayHeight = hasOverlay ? (short)overlayimage.Images[i].Height : (short)0
                        };

                    }
                    else if (hasShadow && !hasOverlay)
                    {
                        //image + shadow only
                        library.Images[i] = new Mir3Library.Mir3Image
                        (Images[i].Image,
                        hasShadow ? shadowimage.Images[i].Image : null,
                        null
                        )
                        {
                            OffSetX = Images[i].X,
                            ...
                            ShadowOffSetX = Images[i].ShadowX,
                            ShadowOffSetY = Images[i].ShadowY,
                            ShadowType = hasShadow ? (byte)1 : (byte)0,
                        };
                    }
                    else if (hasShadow && hasOverlay)
                    {
        ...
        public class WeMadeImage
        {
            ...
            private byte[][] DecompressWemadeMir3(BinaryReader BReader, short OutputWidth, short OutputHeight, int InputLength)
            {
                ...
                try
                {
                    ...
                    for (int Y = OutputHeight + 1; Y > -1; Y--)//wil decode pixel offset fix
                    {
                        OffSet = Start * 2;
                ...
                {
                    System.Console.WriteLine("Error WIL: " + ex.ToString());
                }
            ...
            public unsafe void CreateTexture(BinaryReader reader, int[] palette)
            {
                ...
                switch (nType)
                {
                    ...
                    case 4://shanda miz file compressed
                        ...
                        var deflateStream = new Ionic.Zlib.ZlibStream(output, Ionic.Zlib.CompressionMode.Decompress);

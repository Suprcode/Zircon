...
* **Group drops and currency:** while group loot is enabled, a monster makes one ordinary drop roll for the group rather than one divided roll per nearby member. The credited kill owner's drop, gold and fortune modifiers govern that roll, and every account in the group at the time of death is snapshotted onto its ground items, so members who arrive later can still see and collect them. Any snapshotted member or companion may immediately collect an unfiltered item into their own destination. With Enable Bag selected, every filtered item enters the shared group bag: Random, Need/Greed and Round Robin distribute it through Share, while Free For All lets any member claim it without Share or the manual-taking permission. With Enable Bag cleared, filtered Random and Round Robin items are awarded immediately, Need/Greed begins immediately and safely queues overlapping pickups, and Free For All sends the item directly to its collector. An unresolved immediate award returns to protected group ground loot without being taxed twice. Later joiners are not eligible, while normal public, guild and group ownership timers still apply to outsiders when `DropVisibleOtherPlayers` is enabled. Personal quest rewards remain limited to members who were nearby at death, and harvest-only rewards retain their personal rolls. Companions also apply their own filters and destination-capacity checks. After guild tax, ordinary currency is split immediately across all current group members; quest-bound currency is not split.

...
using static Shared.Rendering.RenderingPipelineManager;
using Buffer = Silk.NET.Vulkan.Buffer;
using ClientBlendMode = Shared.Rendering.BlendMode;
...
namespace Shared.Rendering.SilkVulkan
{
    public sealed unsafe class SilkVulkanRenderingPipeline : IRenderingPipeline
    {
        ...
        private bool _supportsBcTextures;
        ...
        public bool SupportsBc7Textures => _supportsBcTextures;
        ...
        {
            if (_context?.RenderTarget is not Form renderForm)
                return;
            ...
            renderForm.Location = new Point(x, y);
        }
        ...
        public TextureLock LockTexture(RenderTexture texture, TextureLockMode mode)
        {
            ...
            if (IsNativeCompressedTexture(resource.Format) && mode != TextureLockMode.ReadOnly)
                return CreateNativeCompressedTextureWriteLock(resource, GetBlockSize(resource.Format));
        ...
        public void MemoryClear()
        {
            ...
            {
                ITextureCacheItem image = _textureCache[i];
                if (image == null)
                {
            ...
            {
                ISoundCacheItem sound = _soundCache[i];
                if (sound == null)
                {
        ...
        private void PickPhysicalDevice()
        {
            ...
            string requestedAdapterId = string.Empty;
            ...
            _supportsBcTextures = features.TextureCompressionBC && SupportsSampledOptimalFormat(_physicalDevice, Format.BC7UnormBlock);
        }
        ...
        private bool SupportsSampledOptimalFormat(PhysicalDevice device, Format format)
        {
            _vk.GetPhysicalDeviceFormatProperties(device, format, out FormatProperties properties);
            return (properties.OptimalTilingFeatures & FormatFeatureFlags.SampledImageBit) != 0;
        }
            ...
            try
            {
                ...
                fixed (byte** extensionPtr = extensions)
                {
                    ...
                    PhysicalDeviceFeatures features = new PhysicalDeviceFeatures
                    {
                        ...
                        TextureCompressionBC = _supportsBcTextures
                    };
        ...
        private TextureLock CreateNativeCompressedTextureWriteLock(SilkVulkanTextureResource resource, int blockSize)
        {
            int compressedSize = GetCompressedDataSize(resource.Size.Width, resource.Size.Height, blockSize);
            byte[] compressed = new byte[compressedSize];
            GCHandle handle = GCHandle.Alloc(compressed, GCHandleType.Pinned);
            ...
            return TextureLock.From(handle.AddrOfPinnedObject(), compressedSize, () =>
            {
                try
                {
                    UploadTexture(resource, compressed);
                }
                finally
                {
                    handle.Free();
        ...
        private bool ApplyWindowBounds(bool forceCenter = false)
        {
            ...
            if (RenderingPipelineManager.HostSettings.FullScreen)
            {
                ...
                renderForm.StartPosition = FormStartPosition.Manual;
                renderForm.Bounds = RenderingPipelineManager.GetMonitorDisplayBounds(deviceName, screen.Bounds);
                return true;
            }
        ...
        {
            if (!_supportsBcTextures)
                return Format.B8G8R8A8Unorm;

            return format switch
            {
                RenderTextureFormat.Dxt1 => Format.BC1RgbaUnormBlock,
                RenderTextureFormat.Dxt5 => Format.BC3UnormBlock,
                RenderTextureFormat.Bc7 => Format.BC7UnormBlock,
                _ => Format.B8G8R8A8Unorm
            };
        }

        private bool IsNativeCompressedTexture(RenderTextureFormat format)
        {
            return _supportsBcTextures && IsCompressedTextureFormat(format);
        }

        private static bool IsCompressedTextureFormat(RenderTextureFormat format)
        {
            return format == RenderTextureFormat.Dxt1 ||
                   format == RenderTextureFormat.Dxt5 ||
                   format == RenderTextureFormat.Bc7;
        }

        private static int GetBlockSize(RenderTextureFormat format)
        {
            return format == RenderTextureFormat.Dxt1 ? 8 : 16;
        }

        private static int GetCompressedDataSize(int width, int height, int blockSize)
        {

...
namespace Shared.Rendering.SharpDXD3D11
{
    ...
    public static class SharpDXD3D11Manager
    {
        ...
        public static List<ITextureCacheItem> ControlList { get; } = new List<ITextureCacheItem>();
        public static List<ITextureCacheItem> TextureList { get; } = new List<ITextureCacheItem>();
        public static List<ISoundCacheItem> SoundList { get; } = new List<ISoundCacheItem>();
        ...
        public static void Create()
        {
            ...
            if (RenderingPipelineManager.RenderTarget != null && RenderingPipelineManager.RenderTarget.ClientSize != RenderingPipelineManager.HostSettings.GameSize)
                RenderingPipelineManager.RenderTarget.ClientSize = RenderingPipelineManager.HostSettings.GameSize;
            ...
            Size targetSize = RenderingPipelineManager.RenderTarget?.ClientSize ?? RenderingPipelineManager.HostSettings.GameSize;
            ...
            SwapChainDescription swapChainDescription = new SwapChainDescription
            {
                ...
                BufferCount = 1,
                OutputHandle = RenderingPipelineManager.RenderTarget.Handle,
                IsWindowed = true,
            ...
            Factory.MakeWindowAssociation(RenderingPipelineManager.RenderTarget.Handle, WindowAssociationFlags.IgnoreAltEnter);
        ...
        public static void ToggleFullScreen()
        {
            ...
            bool enteringFullScreen = !RenderingPipelineManager.HostSettings.FullScreen;
            Screen selectedScreen = RenderingPipelineManager.GetSelectedScreen();
            Point? cursorOffset = CaptureCursorOffset(selectedScreen);

            RenderingPipelineManager.HostSettings.FullScreen = enteringFullScreen;
            ...
            RequestReset();
            ApplyPostTogglePlacement(RenderingPipelineManager.HostSettings.FullScreen, selectedScreen, cursorOffset);
        }

        public static void SetResolution(Size size)
        {
            if (RenderingPipelineManager.RenderTarget.ClientSize == size && GetBackBufferSize() == size)
                return;

            RenderingPipelineManager.HostSettings.GameSize = size;

            if (!RenderingPipelineManager.HostSettings.FullScreen && RenderingPipelineManager.RenderTarget != null && RenderingPipelineManager.RenderTarget.ClientSize != size)
            {
                RenderingPipelineManager.RenderTarget.ClientSize = size;
                CenterOnSelectedMonitor();
            }
        ...
        {
            if (RenderingPipelineManager.RenderTarget is not Form renderForm)
                return;
            ...
            Size size = RenderingPipelineManager.HostSettings.ActiveSceneSize;
            if (!RenderingPipelineManager.HostSettings.FullScreen && renderForm.ClientSize != size)
                renderForm.ClientSize = size;

            GdiRectangle bounds = screen.Bounds;

            int x = bounds.Left + (bounds.Width - renderForm.Width) / 2;
            int y = bounds.Top + (bounds.Height - renderForm.Height) / 2;

            renderForm.StartPosition = FormStartPosition.Manual;
            renderForm.Location = new GdiPoint(x, y);
        }
        ...
        private static void ApplyPostTogglePlacement(bool fullScreen, Screen screen, Point? cursorOffset)
        {
            ...
            if (RenderingPipelineManager.RenderTarget == null || RenderingPipelineManager.RenderTarget.IsDisposed || !RenderingPipelineManager.RenderTarget.IsHandleCreated)
                return;

            try
            {
                RenderingPipelineManager.RenderTarget.BeginInvoke((MethodInvoker)(() =>
                {
                    ApplyTogglePlacement(fullScreen, screen, cursorOffset);
        ...
        {
            if (RenderingPipelineManager.RenderTarget == null || RenderingPipelineManager.RenderTarget.IsDisposed)
                return;
        ...
        private static void ApplyWindowStyle()
        {
            ...
            renderForm.FormBorderStyle = (RenderingPipelineManager.HostSettings.FullScreen || RenderingPipelineManager.HostSettings.Borderless) ? FormBorderStyle.None : FormBorderStyle.FixedDialog;
            renderForm.MaximizeBox = false;
        }

        private static bool ApplyWindowBounds(bool forceCenter = false)
        {
            if (RenderingPipelineManager.RenderTarget == null)
                return true;
            ...
            if (RenderingPipelineManager.HostSettings.FullScreen)
            {
                ...
                if (!_displayMode.Apply(selectedScreen, RenderingPipelineManager.HostSettings.GameSize))
                    return false;

                selectedScreen = DisplayModeManager.GetScreenByDeviceName(deviceName, RenderingPipelineManager.GetSelectedScreen());
                renderForm.StartPosition = FormStartPosition.Manual;
                renderForm.Bounds = RenderingPipelineManager.GetMonitorDisplayBounds(deviceName, selectedScreen.Bounds);
                return true;
            }
            ...
            Size size = RenderingPipelineManager.HostSettings.ActiveSceneSize;
            if (RenderingPipelineManager.RenderTarget.ClientSize != size)
                RenderingPipelineManager.RenderTarget.ClientSize = size;

            if (forceCenter || !IsWindowOnScreen(RenderingPipelineManager.GetSelectedScreen()))
                CenterOnSelectedMonitor();
        ...
        {
            if (screen == null || RenderingPipelineManager.RenderTarget == null)
                return false;

            Rectangle bounds = RenderingPipelineManager.GetMonitorDisplayBounds(screen);
            return bounds.IntersectsWith(RenderingPipelineManager.RenderTarget.Bounds);
        }
        ...
        {
            BitmapInterpolationMode interpolationMode = mode switch
            {
            ...
            if (InterpolationMode == interpolationMode)
                return;
        ...
        public static void MemoryClear()
        {
            ...
            {
                if (RenderingPipelineManager.HostSettings.CurrentTime < ControlList[i].ExpireTime)
                    continue;
            ...
            {
                if (RenderingPipelineManager.HostSettings.CurrentTime < TextureList[i].ExpireTime)
                    continue;
            ...
            {
                if (RenderingPipelineManager.HostSettings.CurrentTime < SoundList[i].ExpireTime)
                    continue;
        ...
        public static void SetRenderTarget(SharpD3D11RenderTarget target)
        {
            ...
            if (target.IsDisposed)
                throw new ObjectDisposedException(nameof(SharpD3D11RenderTarget));
        ...
        public static void EndDraw()
        {
            ...
            SwapChain.Present(RenderingPipelineManager.HostSettings.VSync ? 1 : 0, PresentFlags.None);
        }
        ...
        public static void ReleaseRenderTarget(SharpD3D11RenderTarget renderTarget)
        {
            ...
            if (ReferenceEquals(CurrentTarget, renderTarget))
            {
                CurrentTarget = _backBufferTarget != null && !_backBufferTarget.IsDisposed ? _backBufferTarget : null;

                if (CurrentTarget != null && Context != null && D2DContext != null)
                {
                    Context.OutputMerger.SetRenderTargets(CurrentTarget.RenderTargetView);
                    D2DContext.Target = CurrentTarget.TargetBitmap;
                }
            }
        ...
        private static void RecreateSwapChain(Size size)
        {
            ...
            Size targetSize = RenderingPipelineManager.RenderTarget?.ClientSize ?? size;

...
namespace Shared.Rendering.SharpDXD3D11
{
    public sealed class SharpDXD3D11RenderingPipeline : IRenderingPipeline
    {
        ...
        public void QueueSprite(RenderTexture texture, Rectangle sourceRectangle, RectangleF destinationRectangle, Color colour)
        {
            ...
            if (!RenderingPipelineManager.HostSettings.UseD3D11SpriteBatch ||
        ...
        private void EndSpriteBatch(string reason)
        {
            ...
            SharpD3D11RenderTarget currentTarget = SharpDXD3D11Manager.CurrentTarget;
            var targetTexture = currentTarget?.Texture;
            if (currentTarget != null && !currentTarget.IsDisposed && targetTexture != null && !targetTexture.IsDisposed)
            {
        ...
        public void ReleaseRenderTarget(RenderTargetResource renderTarget)
        {
            ...
            {
                EndSpriteBatch("ReleaseRenderTarget");
                FlushLineBatch();
        ...
        {
            if (control != null && !SharpDXD3D11Manager.ControlList.Contains(control))
                SharpDXD3D11Manager.ControlList.Add(control);
        }

        public void UnregisterControlCache(ITextureCacheItem control)
        {
            if (control != null)
                SharpDXD3D11Manager.ControlList.Remove(control);
        }
        ...
        {
            if (texture != null && !SharpDXD3D11Manager.TextureList.Contains(texture))
                SharpDXD3D11Manager.TextureList.Add(texture);
        }

        public void UnregisterTextureCache(ITextureCacheItem texture)
        {
            if (texture != null)
                SharpDXD3D11Manager.TextureList.Remove(texture);
        }

        public void RegisterSoundCache(ISoundCacheItem sound)
        {
            if (sound != null && !SharpDXD3D11Manager.SoundList.Contains(sound))
                SharpDXD3D11Manager.SoundList.Add(sound);
        }

        public void UnregisterSoundCache(ISoundCacheItem sound)
        {
            if (sound != null)
                SharpDXD3D11Manager.SoundList.Remove(sound);
        }
        ...
        public IReadOnlyList<ISoundCacheItem> GetRegisteredSoundCaches()
        {
            ...
            return SharpDXD3D11Manager.SoundList;
        }

...
namespace Shared.Rendering.SharpDXD3D9
{
    public sealed class SharpDXD3D9RenderingPipeline : IRenderingPipeline
    {
        ...
        {
            if (control != null && !SharpDXD3D9Manager.ControlList.Contains(control))
                SharpDXD3D9Manager.ControlList.Add(control);
        }

        public void UnregisterControlCache(ITextureCacheItem control)
        {
            if (control != null)
                SharpDXD3D9Manager.ControlList.Remove(control);
        }

        public void RegisterTextureCache(ITextureCacheItem texture)
        {
            if (texture != null && !SharpDXD3D9Manager.TextureList.Contains(texture))
                SharpDXD3D9Manager.TextureList.Add(texture);
        }

        public void UnregisterTextureCache(ITextureCacheItem texture)
        {
            if (texture != null)
                SharpDXD3D9Manager.TextureList.Remove(texture);
        }

        public void RegisterSoundCache(ISoundCacheItem sound)
        {
            if (sound != null && !SharpDXD3D9Manager.SoundList.Contains(sound))
                SharpDXD3D9Manager.SoundList.Add(sound);
        }

        public void UnregisterSoundCache(ISoundCacheItem sound)
        {
            if (sound != null)
                SharpDXD3D9Manager.SoundList.Remove(sound);
        }
        ...
        public IReadOnlyList<ISoundCacheItem> GetRegisteredSoundCaches()
        {
            ...
            return SharpDXD3D9Manager.SoundList;
        }
